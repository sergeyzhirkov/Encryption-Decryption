...
public class EncryptShift implements EncryptMethod {
    ...
    public StringBuilder enc(String message, int key) {
        ...
        for (int i = 0; i < message.length(); i++) {
            ...
            if (ch >= 'a' && ch <= 'z') {
                int position = (ENGLISH_ALPHABET_L.indexOf(ch) + key) % ENGLISH_ALPHABET_L.length();
                ch = ENGLISH_ALPHABET_L.charAt(position);
            } else if (ch >= 'A' && ch <= 'Z') {
                int position = (ENGLISH_ALPHABET_U.indexOf(ch) + key) % ENGLISH_ALPHABET_U.length();
